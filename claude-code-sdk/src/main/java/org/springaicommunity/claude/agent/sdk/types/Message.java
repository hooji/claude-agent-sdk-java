 ...
 * parsing mechanism —
 * {@link org.springaicommunity.claude.agent.sdk.parsing.MessageParser} hand-parses the
 * CLI wire format, whose nested shape doesn't bind to these records — and the
 * registration had gone stale: it predated {@code StreamEvent} and the replay-only types
 * ({@code ForkMarker}, {@code HistoryEnd}, {@code RawTranscriptMessage}, the latter with
 * a dynamic type id that NAME-based polymorphism cannot express). Serializing those as
 * {@code Message} produced duplicate, conflicting {@code "type"} keys and output that
