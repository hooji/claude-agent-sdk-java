...
| **Session cloning** (`SessionClone`) | Clones a session into a new working directory — conversation, file state, *and* metadata together — unlike `--fork-session`, which branches the conversation but shares one directory. | [docs/session-history.md](docs/session-history.md) |
| **Session archives** (`SessionArchive`) | Packages one session — transcript, its `.meta` metadata, *and* its whole working-directory tree — into a single portable ZIP, restorable to a new directory while keeping (or replacing) the session id. | [docs/session-history.md](docs/session-history.md) |
