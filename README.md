...
| **Auth status & token identity** (`ClaudeAuth`) | Read which Anthropic account the CLI is signed in as — `email`, `orgId`, `orgName`, `subscriptionType` on interactive claude.ai logins; auth state (`loggedIn`, `authMethod`, `apiProvider`) under token/API-key auth — via the supported `claude auth status --json` subcommand (local, instant, typed `AuthStatus` with `allValues()`). Plus `profile(oauthToken)`: resolve an OAuth token to its account (email, UUIDs) via the undocumented `/api/oauth/profile` endpoint the CLI itself uses internally — requires the `user:profile` scope, which login access tokens carry and `setup-token` tokens lack. | [Auth Status](#auth-status) |
...
For the token-auth case — "whose token is this?" — `ClaudeAuth.profile(oauthToken)` resolves an OAuth token to its account (email, account/org UUIDs, display names) via `GET /api/oauth/profile`, the same endpoint the CLI itself uses internally to resolve identity during login. **Undocumented API**: shape and availability can change without notice, and the endpoint requires the `user:profile` scope — the interactive login's access token carries it (`ClaudeCloudSessions.getClaudeOAuthToken()`), while long-lived `claude setup-token` tokens do not (they are rejected with `oauth_scope_insufficient`, surfaced as a descriptive exception):

```java
var profile = ClaudeAuth.profile(ClaudeCloudSessions.getClaudeOAuthToken());
