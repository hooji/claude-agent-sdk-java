...
| **Raw API body logging** (`CLIOptions.otelLogRawApiBodiesDirectory`) | Sets the CLI's `OTEL_LOG_RAW_API_BODIES` environment variable to `file:<directory>`, so the CLI writes untruncated request/response JSON for every Anthropic Messages API call into that directory. Still requires telemetry to be enabled separately (`CLAUDE_CODE_ENABLE_TELEMETRY=1` + an `OTEL_LOGS_EXPORTER`). | — |
