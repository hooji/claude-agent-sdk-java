...
| **Session archives** (`SessionArchive`) | Packages one session — transcript *and* its whole working-directory tree — into a single portable ZIP with metadata (`Map<String,Serializable>` attributes), restorable to a new directory while keeping (or replacing) the session id. | [docs/session-history.md](docs/session-history.md) |
